...
# Trong môi trường thực tế, khóa này phải được quản lý an toàn và không tạo mới mỗi khi ứng dụng khởi động lại
...
<body class="min-h-screen flex items-center justify-center p-4">
  <div class="max-w-4xl mx-auto p-8 bg-white rounded-3xl shadow-2xl space-y-10 w-full transform transition-all duration-300 ease-in-out hover:shadow-3xl">
    ...
    <div class="border border-red-300 rounded-2xl p-7 bg-gradient-to-br from-red-50 to-red-100 shadow-lg">
      <h2 class="text-2xl font-bold text-red-800 mb-5 flex items-center gap-3">
        <span class="text-3xl">🔄</span> Chức năng Tự Động Truyền - Nhận và Xác minh
      </h2>
      <p class="text-base text-gray-700 mb-5 leading-relaxed">
        Sử dụng chức năng này để kiểm tra quy trình ký và xác minh tự động trong ứng dụng.
        Tệp bạn tải lên sẽ được ký và ngay lập tức được xác minh lại bằng chữ ký và public key của chính nó.
        Đây là một cách nhanh chóng để kiểm tra tính toàn vẹn của hệ thống.
      </p>
      <form method="POST" enctype="multipart/form-data" action="/auto_transfer_receive" class="space-y-5">
        <div>
          <label for="file_auto_process" class="file-input-label">
            <span class="mr-2 text-xl">🚀</span> Chọn tệp để tự động xử lý
            <span id="file_auto_process_name" class="ml-2 text-gray-600 font-normal italic"></span>
          </label>
          <input type="file" name="file" id="file_auto_process" class="hidden" required onchange="document.getElementById('file_auto_process_name').innerText = this.files[0].name || 'Chưa có tệp nào được chọn'">
        </div>
        <button type="submit" class="w-full bg-red-700 text-white px-6 py-3 rounded-xl hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 focus:ring-offset-2 transition-all duration-300 text-lg font-semibold shadow-md hover:shadow-lg transform hover:scale-105">
          Ký và Tự Động Xác Minh
          <span class="ml-2 text-xl">⚡</span>
        </button>
      </form>
      {% if auto_verify_message %}
      <p class="message-box {% if 'thất bại' in auto_verify_message %}message-error{% else %}message-success{% endif %}">
        {% if 'thất bại' in auto_verify_message %}<span class="text-xl">❌</span>{% else %}<span class="text-xl">✅</span>{% endif %} {{ auto_verify_message }}
      </p>
      {% endif %}
    </div>
    ...
    <div class="border border-purple-300 rounded-2xl p-7 bg-gradient-to-br from-purple-50 to-purple-100 shadow-lg">
      <h2 class="text-2xl font-bold text-purple-800 mb-5 flex items-center gap-3">
        <span class="text-3xl">📦</span> Nhận và xác minh file (Thủ công)
      </h2>
      <p class="text-base text-gray-700 mb-5 leading-relaxed">
...
@app.route("/auto_transfer_receive", methods=["POST"])
def auto_transfer_receive():
    file = request.files.get("file")

    if not file:
        return render_template_string(HTML, auto_verify_message="❌ Lỗi: Vui lòng chọn tệp để tự động xử lý.")

    # Bước 1: Ký file (Giống như sign_and_get_details)
    filepath_signed = os.path.join(UPLOAD_FOLDER, file.filename)
    try:
        file.save(filepath_signed) # Lưu bản gốc vào thư mục uploads
    except Exception as e:
        return render_template_string(HTML, auto_verify_message=f"❌ Lỗi khi lưu tệp gốc để ký: {e}")

    with open(filepath_signed, "rb") as f:
        data_to_sign = f.read()

    digest_to_sign = hashlib.sha512(data_to_sign).digest()
    try:
        signature = private_key.sign(
            digest_to_sign,
            padding.PKCS1v15(),
        ...
        signature_b64 = base64.b64encode(signature).decode()
        # Public key đã được lưu trong biến global public_pem
    except Exception as e:
        return render_template_string(HTML, auto_verify_message=f"❌ Lỗi khi tạo chữ ký trong quá trình tự động: {e}")

    # Bước 2: Tự động "nhận" và xác minh file
    # Trong trường hợp này, file "nhận" chính là file vừa được "gửi" (đã lưu trong UPLOAD_FOLDER)
    filepath_received = filepath_signed # Sử dụng cùng một đường dẫn file

    with open(filepath_received, "rb") as f:
        data_to_verify = f.read()
    digest_to_verify = hashlib.sha512(data_to_verify).digest()
    
    try:
        received_signature = base64.b64decode(signature_b64)
    except Exception as e:
        return render_template_string(HTML, auto_verify_message=f"❌ Xác minh tự động thất bại: Chữ ký không hợp lệ (không phải Base64): {e}")

    try:
        # Sử dụng public_key đã tạo ở đầu ứng dụng
        public_key_obj = serialization.load_pem_public_key(public_pem.encode())
        public_key_obj.verify(received_signature, digest_to_verify, padding.PKCS1v15(), hashes.SHA512())
        auto_verify_msg = f"✅ Tự động xác minh thành công! File '{file.filename}' đã được ký và xác minh lại chính xác."
    except InvalidSignature:
        auto_verify_msg = "❌ Tự động xác minh thất bại: Chữ ký không khớp với dữ liệu hoặc public key. File có thể đã bị thay đổi hoặc chữ ký/public key không đúng."
    except ValueError as e:
        auto_verify_msg = f"❌ Tự động xác minh thất bại: Public key không hợp lệ (không phải định dạng PEM hoặc lỗi khác): {e}"
    except Exception as e:
        auto_verify_msg = f"❌ Tự động xác minh thất bại: Xảy ra lỗi không xác định trong quá trình xác minh: {e}"

    return render_template_string(HTML, auto_verify_message=auto_verify_msg)


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, debug=True) # Thêm debug=True để dễ phát triển